...
class McpAuthInterestRegistrationTest {
    ...
    void createSessionRegistersMcpAuthInterestOnlyWhenHandlerConfigured() throws Exception {
        try (var server = new RecordingRuntime();
            ...
            try (var session = client.createSession(new SessionConfig()
                    .setOnPermissionRequest(PermissionHandler.APPROVE_ALL)
                    .setOnMcpAuthRequest(request -> java.util.concurrent.CompletableFuture
                            .completedFuture(McpAuthResult.cancelled()))).get()) {
                assertNotNull(session);
            }
            ...
            assertEquals("session.create", requests.get(1).method());
        }
    }
    ...
    void cloudCreateSessionRegistersMcpAuthInterestAfterCreateOnlyWhenHandlerConfigured() throws Exception {
        try (var server = new RecordingRuntime();
            ...
            var cloud = new CloudSessionOptions()
                    .setRepository(new CloudSessionRepository().setOwner("github").setName("copilot-sdk").setBranch("main"));
    ...
    void resumeSessionRegistersMcpAuthInterestOnlyWhenHandlerConfigured() throws Exception {
        try (var server = new RecordingRuntime();
            ...
            try (var session = client.resumeSession("session-with-auth", new ResumeSessionConfig()
                    .setOnPermissionRequest(PermissionHandler.APPROVE_ALL)
                    .setOnMcpAuthRequest(request -> java.util.concurrent.CompletableFuture
                            .completedFuture(McpAuthResult.cancelled()))).get()) {
                assertNotNull(session);
