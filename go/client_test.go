...
func TestClient_MCPAuthInterestRegistration(t *testing.T) {
	...
	t.Run("create registers MCP OAuth interest before local session create when auth handler is configured", func(t *testing.T) {
		client, requests, cleanup := newInMemoryClient(t)
		defer cleanup()
		...
		session, err := client.CreateSession(t.Context(), &SessionConfig{
			...
			OnMCPAuthRequest: func(MCPAuthRequest, MCPAuthInvocation) (*MCPAuthResult, error) {
				return &MCPAuthResult{Kind: "cancelled"}, nil
			},
		})
		if err != nil {
			t.Fatalf("CreateSession failed: %v", err)
		...
		if snapshot[0].Method != "session.eventLog.registerInterest" {
			t.Fatalf("expected MCP auth interest before session.create, got %s", snapshot[0].Method)
		}
		if snapshot[1].Method != "session.create" {
			t.Fatalf("expected session.create after MCP auth interest, got %s", snapshot[1].Method)
		}
